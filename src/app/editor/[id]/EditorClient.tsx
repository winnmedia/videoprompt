...
export default function EditorClient({ id }: EditorClientProps) {
  ...
  const handleDeleteBead = (beadId: string) => {
    const updatedTimeline = timeline.filter(b => b.id !== beadId);
    setTimeline(updatedTimeline);
    const newTotal = updatedTimeline.reduce((t, b) => t + b.duration, 0);
    setTotalDuration(newTotal);
    if (selectedBead?.id === beadId) setSelectedBead(null);
  };

  const handleBeadDurationChange = (beadId: string, newDuration: number) => {
    const updatedTimeline = timeline.map(b => b.id === beadId ? { ...b, duration: newDuration, endTime: b.startTime + newDuration } : b);
    setTimeline(updatedTimeline);
    const newTotal = updatedTimeline.reduce((t, b) => t + b.duration, 0);
    setTotalDuration(newTotal);
  };
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6">
                  ...
                  {timeline.map((bead) => (
                    <div
                      ...
                      className={`absolute top-0 h-16 rounded-lg border-2 cursor-pointer transition-all ${
                        selectedBead?.id === bead.id ? 'border-primary-500 bg-primary-50' : 'border-gray-300 bg-white hover:border-gray-400'
                      }`}
                      style={{
                        left: `${(bead.startTime / Math.max(totalDuration, 1)) * 100}%`,
                        width: `${(bead.duration / Math.max(totalDuration, 1)) * 100}%`,
                      }}
                      onClick={() => handleBeadClick(bead)}
                    >
                      <div className="p-2 h-full flex flex-col justify-between">
                        <div className="text-xs font-medium text-gray-700">구슬 {bead.sequence}</div>
                        <div className="text-xs text-gray-500">{bead.duration}초</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
                <div className="text-center">
                  ...
                  <div className="text-lg font-bold text-primary-600">{formatTime(totalDuration)}</div>
                </div>
                <div className="text-center">
                  <span className="font-medium text-gray-700">구슬 개수</span>
                  <div className="text-lg font-bold text-gray-600">{timeline.length}</div>
                </div>
                <div className="text-center">
                  <span className="font-medium text-gray-700">현재 시간</span>
                  <div className="text-lg font-bold text-red-600">{formatTime(currentTime)}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            {selectedBead ? (
              <div className="bg-white rounded-lg shadow-sm border p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-gray-900">구슬 {selectedBead.sequence} 편집</h3>
                  <Button variant="outline" size="sm" onClick={() => handleDeleteBead(selectedBead.id)} className="text-red-600 hover:text-red-700">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">{formatTime(selectedBead.startTime)}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">종료 시간</label>
                    <div className="text-sm text-gray-600">{formatTime(selectedBead.endTime)}</div>
                  </div>
                </div>
                ...
                <div className="mt-6">
                  ...
                  {selectedBead.actions.length > 0 ? (
                    ...
                    <div className="text-sm text-gray-500 text-center py-4">액션이 없습니다</div>
                  )}
                  <Button variant="outline" size="sm" className="w-full mt-3">
                    <Icon name="plus" size="sm" className="mr-2" />액션 추가
                  </Button>
                </div>
                ...
                <div className="mt-6">
                  ...
                  {selectedBead.audio.length > 0 ? (
                    ...
                    <div className="text-sm text-gray-500 text-center py-4">오디오가 없습니다</div>
                  )}
                  <Button variant="outline" size="sm" className="w-full mt-3">
                    <Icon name="plus" size="sm" className="mr-2" />오디오 추가
                  </Button>
                </div>
              </div>
            ) : (
