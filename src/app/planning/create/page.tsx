...
const DEVELOPMENT_METHODS = [
  { value: 'hook-immersion-reversal', label: '훅–몰입–반전–떡밥', description: '강한 시작, 몰입, 반전, 다음 편 유도' },
  ...
  { value: 'documentary-interview', label: '다큐(인터뷰식)', description: '인터뷰 중심의 다큐멘터리' },
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="text-slate-600 mt-2">체계적인 영상 기획을 위한 3단계 위저드</p>
            </div>
            <button
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          {currentStep === 1 && (
            <div className="space-y-8">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-slate-900 mb-4">기본 정보 입력 및 선택</h2>
                <p className="text-slate-600">영상의 기본 정보를 입력하고 주요 설정을 선택해주세요</p>
              </div>
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                {/* 프리셋 버튼들 */}
                <div className="mt-8">
                  <h3 className="text-lg font-semibold text-slate-800 mb-4">🚀 빠른 프리셋</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {PRESET_OPTIONS.map((preset, index) => (
                      <button
                        key={index}
                        onClick={() => applyPreset(preset)}
                        className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl hover:from-blue-100 hover:to-indigo-100 hover:border-blue-300 transition-all duration-200 text-left"
                      >
                        <h4 className="font-semibold text-blue-800 mb-1">{preset.name}</h4>
          ...
          {currentStep === 2 && (
            <div className="space-y-8">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-slate-900 mb-4">스토리 전개 방식 설정</h2>
              ...
              <div className="max-w-4xl mx-auto space-y-8">
                {/* 전개 방식 선택 */}
                <div>
                  <h3 className="text-lg font-semibold text-slate-800 mb-4">📖 전개 방식 *</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {DEVELOPMENT_METHODS.map((method) => (
                      ...
                      >
                        <h4 className="font-semibold text-slate-900 mb-2">{method.label}</h4>
                ...
                {/* 전개 강도 선택 */}
                <div>
                  <h3 className="text-lg font-semibold text-slate-800 mb-4">⚡ 전개 강도 *</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {DEVELOPMENT_INTENSITIES.map((intensity) => (
                      ...
                      >
                        <h4 className="font-semibold text-slate-900 mb-2">{intensity.label}</h4>
                ...
                <div className="bg-slate-50 rounded-xl p-6">
                  <h4 className="font-semibold text-slate-800 mb-4">📋 선택된 설정 요약</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="space-y-2">
                      <div><strong>제목:</strong> {planningData.title}</div>
                      <div><strong>장르:</strong> {GENRE_OPTIONS.find(g => g.value === planningData.genre)?.label}</div>
                      <div><strong>톤앤매너:</strong> {TONE_OPTIONS.find(t => t.value === planningData.tone)?.label}</div>
                      <div><strong>분량:</strong> {planningData.duration}초</div>
                    </div>
                    <div className="space-y-2">
                      <div><strong>포맷:</strong> {FORMAT_OPTIONS.find(f => f.value === planningData.format)?.label}</div>
                      <div><strong>템포:</strong> {TEMPO_OPTIONS.find(t => t.value === planningData.tempo)?.label}</div>
                      <div><strong>전개 방식:</strong> {DEVELOPMENT_METHODS.find(d => d.value === planningData.developmentMethod)?.label}</div>
                      <div><strong>전개 강도:</strong> {DEVELOPMENT_INTENSITIES.find(d => d.value === planningData.developmentIntensity)?.label}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-center space-x-4">
                ...
                <button
                  ...
                  disabled={!planningData.developmentMethod || !planningData.developmentIntensity || isGenerating}
                  className="px-8 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors text-lg"
                >
                  {isGenerating ? (
          ...
          {currentStep === 3 && (
            <div className="space-y-8">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-slate-900 mb-4">기획안 완성 및 확인</h2>
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                {generatedPlan && (
                  <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-6 border border-green-200">
                    <h3 className="font-bold text-green-800 mb-4 text-lg">🎯 AI가 생성한 기획안</h3>
                    
                    <div className="bg-white rounded-lg p-4 mb-4">
                      <h4 className="font-semibold text-slate-800 mb-2">기획 요약</h4>
                      <p className="text-slate-700">{generatedPlan.summary}</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <h4 className="font-semibold text-slate-800 mb-3">📋 스토리 구조</h4>
                        <div className="space-y-2">
                      ...
                      <div>
                        <h4 className="font-semibold text-slate-800 mb-3">🎨 시각적 스타일</h4>
                        <div className="space-y-2 text-sm text-slate-700">
                          <div><strong>스타일:</strong> {generatedPlan.visualStyle}</div>
                          <div><strong>타겟:</strong> {generatedPlan.targetAudience}</div>
                          <div><strong>예상 비용:</strong> {generatedPlan.estimatedCost}</div>
                          <div><strong>제작 기간:</strong> {generatedPlan.timeline}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {/* 최종 설정 요약 */}
                <div className="bg-slate-50 rounded-xl p-6">
                  <h4 className="font-semibold text-slate-800 mb-4">📊 최종 기획 설정</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div className="space-y-2">
                      <div><strong>제목:</strong> {planningData.title}</div>
                      <div><strong>로그라인:</strong> {planningData.logline}</div>
                      <div><strong>장르:</strong> {GENRE_OPTIONS.find(g => g.value === planningData.genre)?.label}</div>
                      <div><strong>톤앤매너:</strong> {TONE_OPTIONS.find(t => t.value === planningData.tone)?.label}</div>
                      <div><strong>분량:</strong> {planningData.duration}초</div>
                    </div>
                    <div className="space-y-2">
                      ...
                      <div><strong>전개 방식:</strong> {DEVELOPMENT_METHODS.find(d => d.value === planningData.developmentMethod)?.label}</div>
                      <div><strong>전개 강도:</strong> {DEVELOPMENT_INTENSITIES.find(d => d.value === planningData.developmentIntensity)?.label}</div>
                      <div><strong>타겟:</strong> {planningData.target}</div>
                    </div>
                  </div>
                </div>
              </div>
