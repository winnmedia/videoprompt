...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!title || !logline || !genre) {
      return NextResponse.json({ error: '제목, 로그라인, 장르는 필수입니다.' }, { status: 400 });
    }
...
function generateDefaultPlanning(
  ...
  const baseStructure = methodStructures[developmentMethod as keyof typeof methodStructures] ||
    genreStructures[genre as keyof typeof genreStructures] || ['도입', '전개', '위기', '해결'];
  ...
  const estimatedCost =
    costFactors.duration === '높음' ||
    costFactors.format === '높음' ||
    costFactors.intensity === '높음'
      ? '높음'
      : costFactors.duration === '중간' ||
          costFactors.format === '중간' ||
          costFactors.intensity === '중간'
        ? '중간'
        : '낮음';
  ...
  const timelineFactors = {
    ...
    intensity:
      developmentIntensity === 'rich' ? 1.5 : developmentIntensity === 'moderate' ? 1 : 0.8,
  };

  const totalWeeks = Math.ceil(
    timelineFactors.duration * timelineFactors.format * timelineFactors.intensity,
  );
