 ...
 *    logger.info(`Found comment at position ${comment.pos}: '${fullText}'.`);
 * };
 * ```
 */
 ...
 *    logger.info(`Found token at position: ${token.pos}.`);
 * };
 * ```
 */
 ...
 * 	logger.info("Found token:", token.getText());
 * });
 * ```
 ...
 * 	logger.info(`${identifier.getText()} is used ${information.uses.length} time(s).`);
 * }
 * ```
 */
