...
import { Zap, BarChart, Mail, Cloud, Globe, Video, Cpu } from 'lucide-react';
...
export const videoPlanetIntegrations: VideoPlanetIntegration[] = [
  {
    ...
    name: 'OpenAI',
    description:
      'AI 기반 장면 생성 및 프롬프트 최적화를 위한 OpenAI 통합. GPT-4를 활용한 고품질 영상 시나리오 생성.',
  ...
  },
  {
    ...
    description: 'Railway 백엔드 기반 파일 스토리지. PostgreSQL과 연동된 통합 데이터 관리.',
    category: 'Storage & Cloud',
    icon: Cloud,
    color: '#FF9800',
    status: 'connected',
    features: ['PostgreSQL 연동', '통합 관리', '백업 시스템', '보안 관리'],
  },
  {
    id: 'analytics',
    name: 'Analytics',
...
export function getIntegrationsByStatus(
  status: VideoPlanetIntegration['status'],
): VideoPlanetIntegration[] {
...
export function searchIntegrations(query: string): VideoPlanetIntegration[] {
  ...
  return videoPlanetIntegrations.filter(
    (integration) =>
