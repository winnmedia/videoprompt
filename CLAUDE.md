...
6.3. 중대 실패 교훈 (Critical Failure Lessons)

[2025-09-23] 병렬 서브에이전트 코드베이스 붕괴 사건

**사건 개요**: Phase 6에서 병렬 서브에이전트 작업으로 인한 전체 코드베이스 붕괴

- **증상**: TypeScript 오류 30개, 환각 코드 다수, FSD 구조 완전 붕괴, 빌드 불가
- **피해 규모**: 58개 파일 오염, 테스트 322개 중 대부분 무의미
- **근본 원인**: 조율 없는 병렬 작업, 검증 없는 속도 추구

**5대 실패 요인**:

1. **병렬 서브에이전트 충돌**: 동일 파일 동시 수정, 의존성 가정 오류
2. **환각 코드 범람**: 컨텍스트 부족으로 존재하지 않는 모듈 참조
3. **FSD 원칙 포기**: entities에 Redux/localStorage 코드 침투
4. **TDD 포기**: 시간 압박으로 테스트 없는 코드 대량 생성
5. **과속 개발**: 27초/파일 속도로 무분별한 파일 생성

**필수 방지책 (영구 적용)**:

1. **병렬 작업 금지**: 서브에이전트는 반드시 순차적으로 실행
2. **단계별 검증 의무화**:
   - 각 파일 생성 후 즉시 TypeScript 컴파일 확인
   - 10개 파일마다 전체 빌드 테스트
   - FSD 경계 위반 즉시 중단
3. **환각 코드 방지**:
   - 모든 import는 실제 export 확인 후 작성
   - 타입은 추론 금지, 명시적 정의만 사용
   - 존재하지 않는 속성 추가 절대 금지
4. **품질 > 속도 원칙**:
   - 최소 2분/파일 투자
...
**이 기록은 영구 보존되며 모든 AI 에이전트가 준수해야 함**
