...
test('buttons-works: 홈/위저드/에디터/통합 주요 버튼 클릭 무에러', async ({ page }) => {
  ...
  // 위저드 주요 버튼
  await safeClickEnabled(page.getByTestId('generate-btn-side')); // 빈 시나리오면 disabled 확인
  ...
  await expect(page.getByRole('heading', { name: /에디터|Editor/i })).toBeVisible({ timeout: 5000 });
