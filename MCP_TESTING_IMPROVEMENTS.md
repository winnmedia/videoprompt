...
// 폼 자동화 테스트
const formResult = await browserManager.testFormAutomation('http://localhost:3000/contact', {
  username: 'testuser',
  email: 'test@example.com',
});

// 반응형 디자인 테스트
const responsiveResults = await browserManager.testResponsiveDesign('http://localhost:3000', [
...
// 테스트 계획 실행
const result = await sequentialManager.executeTestPlan('user-registration-flow', contextManager);
```
