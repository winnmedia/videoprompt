...
- **출력물**: 4단계 구조, 12개 숏트, 콘티 이미지, 기획안 PDF
- **콘티 이미지 생성** : 생성된 12개 숏트를 기반으로 ByteDance-Seedream-4.0 모델을 사용하여 이미지 생성. 제일 첫 장 이미지 생성 후 두 번째 콘티 이미지는 첫 장 이미지를 참고하여 일관성을 유지하며 제작해야함. (추후 콘티 이미지는 영상 ai 생성의 기초 자산이 돼야함)

### 콘텐츠 관리

- **위치**: `/planning`
- **목적**: 생성된 콘텐츠의 통합 관리
- **관리 콘텐츠** : 생성된 시나리오 텍스트, 프롬프트, AI영상, 업로드된 피드백 영상
...
- **기능 명세**:
  - 피드백 유저 초대 : 링크를 통한 회원, 게스트 초대, 게스트 닉네임 설정하여 피드백 등록.
